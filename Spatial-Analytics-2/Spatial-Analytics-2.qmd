...
In my last article, I explained the fundamentals of Spatial Data Analysis and different types of Spatial Data including points, lines, polygons, and grids. If you want a quick recap, you can find it here:

[Hello World: Introducing Spatial Data](https://muzairaslam.medium.com/hello-world-introducing-spatial-data-6c831e87e83f)

I ended the last article with the promise of covering Points in my next article. In this article, I will make a deep dive into Spatial Objects and Spatial Points in R.
...
```
Points are one of the important data models in R. However before we dive into Spatial Points, I want to give a brief overview of Spatial Objects in R.

## Spatial Objects in R

R developers wrote a special package sp which extends base R classes and methods for spatial data. To get things started download sp by following the command:
...
I will call the Spatial class using the getClass method to get the complete definition of an S4 class in R. This 
includes the slot names and types of their content:

```{r get_class,}
getClass("Spatial")
```

This returns us to two important pieces of information about this class.
...
We see that SpatialPoints extends the Spatial class by adding a **coords** slot, into which a matrix of point coordinates can be inserted. We also see three other classes extended by SpatialPoints:
...
str(cran_mat)
```
Before moving on it is necessary to introduce the coordinate reference system (CRS) class briefly. We saw that the **Spatial** class had two slots. One of them was **Coordinate Reference System (CRS)**. Let us see the slot names and components.
...
In this article, we got an overview of the Spatial and SpatialPoints class. But we are only halfway done. In the next article, I will explain the methods and data frames for spatial points to do some data wrangling.
